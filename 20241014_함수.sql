...
-- MON : 월 약어, MONTH : 월, DY : 요일 약어, DAY : 요일
SELECT TO_CHAR(SYSDATE,'MON MONTH DY DAY') FROM DUAL;
-- NLS_DATE_LANGUAGE = 언어 : 언어 설정
...
-- 숫자 포맷
-- 9 : 숫자 한자리, 자리에 숫자가 없으면 공백으로 채움
-- 0 : 숫자 한자리, 자리에 숫자가 없으면 0으로 채움
...
-- PR : - DLF eO <>로 묶어서 표현, S : + - 부호 표시
SELECT TO_CHAR(-1234567,'S9,999,999'), TO_CHAR(1234567,'S9,999,999'), TO_CHAR(-1234567,'9,999,999PR'), TO_CHAR(1234567,'9,999,999PR') FROM DUAL; 
-- 문자열을 날짜로 변경
...
RANK() OVER(ORDER BY PAGE DESC) AS RANK, 
P.* FROM PERSON P;

-- 현재 행을 기준으로 다음 위치에 해당하는 값을 읽어오는 함수
SELECT P.*, LEAD(PNAME) OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;
SELECT P.*, LEAD(PNAME,2) OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;
SELECT P.*, LEAD(PNAME,2,'데이터 없음') OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;

-- 현재 행을 기준으로 이전 위치에 해당하는 값을 읽어오는 함수
SELECT P.*, LAG(PNAME) OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;
SELECT P.*, LAG(PNAME,2) OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;
SELECT P.*, LAG(PNAME,2,'데이터 없음') OVER(ORDER BY PAGE) AS NEXT_PNAME FROM PERSON P;

-- 학생 테이블의 평점을 기준으로 성적 순위를 출력, 성적순은 내림차순으로 처리, 순위는 건너뛰지 않음
SELECT DENSE_RANK() OVER(ORDER BY STD_SCORE DESC) AS RANK, S.*  FROM STUDENT S;
SELECT DENSE_RANK() OVER(PARTITION BY S.MAJOR_NAME ORDER BY S.STD_SCORE DESC) AS RANK, S.*  FROM STUDENT S;

-- 학생 테이블에서 학생정보 조회 시
-- 학번의 경우 앞에 4자리만 표현하고 나머지 4자리는 마스킹 처리해서 조회
SELECT RPAD(SUBSTR(STD_NO,1,4),8,'*') AS STD_NO, STD_NAME, MAJOR_NAME, STD_SCORE FROM STUDENT;
SELECT SUBSTR(STD_NO,1,4) || '****' AS STD_NO, STD_NAME, MAJOR_NAME, STD_SCORE FROM STUDENT;
SELECT CONCAT(SUBSTR(STD_NO,1,4),'****') AS STD_NO, STD_NAME, MAJOR_NAME, STD_SCORE FROM STUDENT;

-- 사원 테이블에서 데이터 조회 시 연봉 순위를 조회, 입사일은 입사년도만 출력
-- 연봉을 출력시 천단위 기호가 붙게끔 처리
SELECT DENSE_RANK() OVER(ORDER BY EMP_SALARY DESC) AS SALARY_RANK, EMP_NO, TO_CHAR(EMP_COURSE_DATE, 'YYYY') AS COURSE_DATE, TO_CHAR(EMP_SALARY, 'FM999,999,999,999') AS SALARY FROM EMPLOYEE;

-- 학생 테이블에서 성씨별로 점수 순위를 내림 차순 기준으로 조회하시오.
-- 출력 형태는 순위 학번 성씨 학과명 평점
SELECT DENSE_RANK() OVER(PARTITION BY SUBSTR(S.STD_NAME,1,1) ORDER BY S.STD_SCORE DESC) AS SCORE_RANK, S.* FROM STUDENT S WHERE S.STD_SCORE >= 3.0;
-- 입학한 년도 별로 평점 순위 구하기
SELECT DENSE_RANK() OVER(PARTITION BY SUBSTR(S.STD_NO,1,4) ORDER BY S.STD_SCORE DESC) AS SCORE_RANK, S.* FROM STUDENT S WHERE S.STD_SCORE >= 3.0;

ALTER TABLE STUDENT RENAME COLUMN STD_NEW_NAME TO STD_NAME;
