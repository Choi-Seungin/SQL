...
-- 프로시저, 함수, 트리거 등의 형태로 작성할 수 있다
-- 데이터 조작 및 비지니스 로직을 데이터베이스 내에서 직접 처리할 수 있다
-- 함수
CREATE OR REPLACE
FUNCTION GET_ODD_EVEN(N IN NUMBER)
RETURN VARCHAR2
IS
	-- 함수에서 사용할 변수를 선언
	MSG VARCHAR2(100);
...
SELECT GET_MAJOR_NAME('11') FROM DUAL;
...
-- 트리거는 테이블에 종속적이기 때문에 테이블 생성 후 트리거 생성, 삭제
------------------------------------------------------------------------
...
-- MAJOR 테이블에 내용이 UPDATE 되면 해당 기록을 저장하는 트리거는
...
INSERT INTO MAJOR VALUES('C3', '게임기획과');
SELECT * FROM DATA_LOG;
...
DELETE FROM MAJOR WHERE MAJOR_NO = 'C3';
SELECT * FROM DATA_LOG;
...
CREATE USER C##CHOI1 IDENTIFIED BY 123456;
GRANT RESOURCE, CONNECT TO C##CHOI1;
ALTER USER C##CHOI1 DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS;

-- C##CHOI1에 C##CHOI.MAJOR 테이블에 대한 권한 부여
GRANT INSERT, UPDATE, DELETE, SELECT ON C##CHOI.MAJOR TO C##CHOI1;
...
INSERT INTO C##CHOI.MAJOR VALUES('C3', '게임기획과');
UPDATE C##CHOI.MAJOR SET MAJOR_NAME = '게임학과'WHERE MAJOR_NO = 'C3';
DELETE FROM C##CHOI.MAJOR WHERE MAJOR_NO = 'C3';
SELECT * FROM DATA_LOG;
...
-- 게시판 테이블 트리거 생성
-- INSERT, UPDATE, DELETE 에 대응하는 트리거 생성
...
-- sql 문과 제어문을 이용해서, 데이터를 검색, 삽입, 수정, 삭제를 할 수 있음
-- 결과를 외부로 전달할 수 있음.
-- 하나의 트랜잭션 구성 시 사용을 함.
----------------------------------------------------------------
-- 매개변수가 없는 프로시저
...
-- 매개변수가 있는 프로시저
-- IN : 입력 매개변수
CREATE OR REPLACE PROCEDURE PROCEDURE_EX2(V_PID IN VARCHAR2, V_PNAME IN VARCHAR2, V_AGE IN NUMBER)
IS
	TEST_VAR VARCHAR2(100);
BEGIN
	TEST_VAR := 'HELLO WORLD';
...
EXCEPTION
	WHEN OTHERS THEN
		...
		ROLLBACK; -- 문제가 생겼을 시 롤백
END;

BEGIN
	PROCEDURE_EX2('0003', '김씨', 20);
END;

SELECT*FROM PERSON;

-- 값을 외부로 전달하는 프로시저
CREATE OR REPLACE PROCEDURE PROCEDURE_EX3(NUM IN NUMBER, RESULT OUT NUMBER)
IS
	I NUMBER;
	USER_EXCEPTION EXCEPTION;
BEGIN
